			...
			[
				{
					...
					value: function attach() {
						_get(
							_getPrototypeOf(MentionBlot.prototype),
							'attach',
							this
						).call(this);
						if (!this.mounted) {
		...
		_createClass(Mention, [
			...
			},
			{
				...
				value: function highlightItem() {
					...
					for (
						var i = 0;
						i < this.mentionList.childNodes.length;
						i += 1
					) {
						this.mentionList.childNodes[i].classList.remove(
							'selected'
						);
					}
					if (
					...
					if (data && data.length > 0) {
						...
						for (var i = 0; i < data.length; i += 1) {
							...
							if (data[i].disabled) {
								...
								initialSelection = i;
							li.dataset.index = i;
							var renderedItem = this.options.renderItem(
								data[i],
								searchTerm
							);
							...
							if (!data[i].disabled) {
								...
								li.onmousedown =
									this.onItemMouseDown.bind(this);
							} else
								li.onmouseenter =
			...
			},
			{
				...
				value: function nextItem() {
					...
					do {
						increment++;
						newIndex =
							(this.itemIndex + increment) % this.values.length;
						var disabled =
			...
			},
			{
				...
				value: function containerBottomIsNotVisible(
					topPos,
					containerPos
				) {
					var mentionContainerBottom =
						topPos +
						this.mentionContainer.offsetHeight +
						containerPos.top;
					return (
						mentionContainerBottom >
			...
			},
			{
				...
				value: function containerRightIsNotVisible(
					leftPos,
					containerPos
				) {
			...
			},
			{
				...
				value: function setMentionContainerPosition_Normal() {
					...
					var mentionCharPos = this.quill.getBounds(
						this.mentionCharPos
					);
					...
					if (this.options.fixMentionsToQuill) {
						var rightPos = 0;
						this.mentionContainer.style.right = ''.concat(
							rightPos,
							'px'
						);
					} else leftPos += mentionCharPos.left;
					if (
						this.containerRightIsNotVisible(leftPos, containerPos)
					) {
						var containerWidth =
							this.mentionContainer.offsetWidth +
					...
					if (this.options.defaultMenuOrientation === 'top') {
						...
						if (this.options.fixMentionsToQuill)
							topPos =
								-1 * (containerHeight + this.options.offsetTop);
						else
							topPos =
								mentionCharPos.top -
						...
						if (
							this.containerBottomIsNotVisible(
								topPos,
								containerPos
							)
						) {
							var _overMentionCharPos =
								this.options.offsetTop * -1;
							if (!this.options.fixMentionsToQuill) {
			...
			},
			{
				...
				value: function setMentionContainerPosition_Fixed() {
					...
					var fitsTop =
						this.mentionContainer.offsetHeight <= availableSpaceTop;
					var placement;
					if (
						this.options.defaultMenuOrientation === 'top' &&
						fitsTop
					) {
						placement = 'top';
					} else if (
			...
			},
			{
				...
				value: function getTextBeforeCursor() {
					var startPos = Math.max(
						0,
						this.cursorPos - this.options.maxChars
					);
			...
			},
			{
				...
				value: function onSomethingChange() {
					...
					var _getMentionCharIndex = getMentionCharIndex(
						...
						mentionCharIndex =
							_getMentionCharIndex.mentionCharIndex;
					if (
						...
						var mentionCharPos =
							this.cursorPos -
							(textBeforeCursor.length - mentionCharIndex);
